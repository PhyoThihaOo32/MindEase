...
void drawSettings(QPainter &p) {
    ...
    for (int i = 0; i < 8; ++i) {
        const qreal a = i * 45.0 * 3.141592653589793 / 180.0;
        const QPointF inner(12 + 6 * std::cos(a), 12 + 6 * std::sin(a));
        const QPointF outer(12 + 9 * std::cos(a), 12 + 9 * std::sin(a));
...
QString lucideIconNameForToken(const QString &token) {
    ...
    if (t == "settings" || has(token, "⚙")) return "settings";
...
QPixmap lucidePixmap(const QString &tokenOrName, const QColor &stroke, int size) {
    ...
    else if (icon == "settings") drawSettings(p);
    else if (icon == "play") {
        QPainterPath path;
        path.moveTo(8, 5);
        path.lineTo(19, 12);
